...
def main():
    ...
    parser.add_argument('--require-replies', action='store_true', default=False, help='Fail if RESP2/RESP3 replies are missing')
    ...
    if args.page_type == 'command':
        ...
        if args.require_replies and (resp2 is None or resp3 is None):
            if resp2 is None:
                print("Missing RESP2 reply for '%s' in '%s' (if command has no reply, set to '[]')" % (name, args.resp2_replies_json), file=sys.stderr)
            if resp3 is None:
                print("Missing RESP3 reply for '%s' in '%s' (if command has no reply, set to '[]')" % (name, args.resp3_replies_json), file=sys.stderr)
            exit(1)
